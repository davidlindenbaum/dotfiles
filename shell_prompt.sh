...
function __promptline_ps1 {
  ...
  #[ $is_prompt_empty -eq 1 ] && slice_prefix="$slice_empty_prefix"
  # section "y" slices
  #__promptline_wrapper "$(custom_git_branch)" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
...
function __promptline_right_slow {
  ...
  # section "y" slices
  __promptline_wrapper "$(custom_git_branch)" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; }
}
function __promptline {
  ...
  function async() {
      # save to temp file
      printf "%s" "$(__promptline_right_slow)" > "/dev/shm/zsh_prompt_$$"
      # signal parent
      kill -s USR1 $$
  }
  if [[ -n ${ZSH_VERSION-} ]]; then
    ...
    RPROMPT="$(__promptline_right_fast)"'$RPROMPTSLOW'"$reset"
  else
...
function TRAPUSR1() {
    # read from temp file
    RPROMPTSLOW="$(cat /dev/shm/zsh_prompt_$$)"
